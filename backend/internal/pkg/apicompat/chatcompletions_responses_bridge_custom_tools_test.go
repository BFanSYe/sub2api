...
func TestResponsesToChatCompletionsRequest_NamespaceToolChoiceMapsToFlattenedChild(t *testing.T) {
	tests := []struct {
		name       string
		namespace  string
		child      string
		...
		},
		{
			...
			namespace:  "gmail",
			child:      "send",
			toolChoice: json.RawMessage(`{"type":"function","namespace":"gmail","function":{"name":"send"}}`),
		},
		{
			name:       "custom type child name",
			namespace:  "gmail",
			child:      "send",
			toolChoice: json.RawMessage(`{"type":"custom","namespace":"gmail","name":"send"}`),
		},
		{
			name:       "long flattened name",
			namespace:  "very_long_namespace_prefix_for_testing_purposes",
			child:      "and_a_rather_long_tool_name_too",
			toolChoice: json.RawMessage(`{"type":"function","namespace":"very_long_namespace_prefix_for_testing_purposes","name":"and_a_rather_long_tool_name_too"}`),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			out, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
				Model:      "glm-5.2",
				Input:      json.RawMessage(`"hi"`),
				Tools:      []ResponsesTool{{Type: "namespace", Name: tt.namespace, Tools: []ResponsesTool{{Type: "function", Name: tt.child}}}},
				ToolChoice: tt.toolChoice,
			})
			require.NoError(t, err)
			require.Len(t, out.Tools, 1)

			declaredName := out.Tools[0].Function.Name
			assert.Equal(t, flattenNamespaceToolName(tt.namespace, tt.child), declaredName)
			assert.JSONEq(t, `{"type":"function","function":{"name":"`+declaredName+`"}}`, string(out.ToolChoice))
		})
	}
}

func TestResponsesToChatCompletionsRequest_DropsNamelessFunctionOrCustomToolChoice(t *testing.T) {
	for _, toolChoice := range []json.RawMessage{
		json.RawMessage(`{"type":"function"}`),
		json.RawMessage(`{"type":"function","function":{}}`),
		json.RawMessage(`{"type":"custom"}`),
	} {
		out, err := ResponsesToChatCompletionsRequest(&ResponsesRequest{
			Model:      "glm-5.2",
			Input:      json.RawMessage(`"hi"`),
